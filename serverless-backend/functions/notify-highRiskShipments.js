...
const { notifyHighRiskShipments } = require('./helper/shipment');
...
module.exports.handler = async (event) => {
  ...
  try {
    const response = await notifyHighRiskShipments();
    const highRiskCount = JSON.parse(response);
    ...
    return {
      ...
              },
      body: JSON.stringify(highRiskCount),
    };
  } catch (error) {
