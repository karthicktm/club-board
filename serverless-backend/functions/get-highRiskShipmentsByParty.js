...
const { getHighRiskShipmentsByParty } = require('./helper/shipment');
const ShipmentDataNotFoundError = require('./lib/ShipmentDataNotFoundError');

module.exports.handler = async (event) => {
  const { party } = event.pathParameters;
  Log.debug('In the get-highRiskShipments handler');

  try {
    const response = await getHighRiskShipmentsByParty(party.toUpperCase());
