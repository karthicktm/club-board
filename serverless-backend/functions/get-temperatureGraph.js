...
const { getTemperatureGraph } = require('./helper/shipment');
const ShipmentDataIntegrityError = require('./lib/ShipmentDataIntegrityError');

module.exports.handler = async (event) => {
  const { shipmentId } = event.pathParameters;
  Log.debug('In the getTemperatureGraph handler with shipmentId ${shipmentId}');

  try {
    const response = await getTemperatureGraph(shipmentId.toUpperCase());
