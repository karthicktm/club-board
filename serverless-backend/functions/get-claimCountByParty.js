...
const { countClaimsByParty } = require('./helper/shipment');
...
module.exports.handler = async (event) => {
  ...
  try {
    const response = await countClaimsByParty(party.toUpperCase());
    const claimCount = JSON.parse(response);
    ...
    return {
      ...
              },
      body: JSON.stringify(claimCount),
    };
  } catch (error) {
