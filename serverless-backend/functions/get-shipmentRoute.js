...
const { getShipmentRoute } = require('./helper/shipment');
const ShipmentDataNotFoundError = require('./lib/ShipmentDataNotFoundError');

module.exports.handler = async (event) => {
  const { shipmentId } = event.pathParameters;
  Log.debug('In the getShipmentRoute handler with shipmentId ${shipmentId}');

  try {
    const response = await getShipmentRoute(shipmentId.toUpperCase());
