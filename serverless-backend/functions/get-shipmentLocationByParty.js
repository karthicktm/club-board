...
const { getShipmentLocation } = require('./helper/shipment');
const ShipmentDataNotFoundError = require('./lib/ShipmentDataNotFoundError');

module.exports.handler = async (event) => {
  const { party } = event.pathParameters;
  Log.debug('In the getShipmentLocation handler');

  try {
    const response = await getShipmentLocation(party.toUpperCase());
